...
void calcstaples_wilson(Gauge_Conf const *const GC, Geometry const *const geo, GParam const *const param, long r, int i, GAUGE_GROUP *M)
	{
	int j, l;
	long k;
...
void compute_clovers(Gauge_Conf const *const GC, Geometry const *const geo, GParam const *const param, int dir)
	{
	long r;
...
int metropolis(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param, long r, int i)
	...
	if(casuale() < 0.5) { times(&new_link, &rnd_matrix, &(GC->lattice[r][i])); }
	else
		{
...
void update(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param)
	...
	for(dir = 0; dir < STDIM; dir++)
		...
		#pragma omp parallel for num_threads(NTHREADS) private(r)
		#endif
		for(r = 0; r < (param->d_volume) / 2; r++) { heatbath(GC, geo, param, r, dir); }

		#ifdef OPENMP_MODE
		#pragma omp parallel for num_threads(NTHREADS) private(r)
		#endif
		for(r = (param->d_volume) / 2; r < (param->d_volume); r++) { heatbath(GC, geo, param, r, dir); }
		}
	...
	for(dir = 0; dir < STDIM; dir++)
		...
		for(j = 0; j < param->d_overrelax; j++)
			...
			#pragma omp parallel for num_threads(NTHREADS) private(r)
			#endif
			for(r = 0; r < (param->d_volume) / 2; r++) { overrelaxation(GC, geo, param, r, dir); }

			#ifdef OPENMP_MODE
			#pragma omp parallel for num_threads(NTHREADS) private(r)
			#endif
			for(r = (param->d_volume) / 2; r < (param->d_volume); r++) { overrelaxation(GC, geo, param, r, dir); }
			}
		}
...
void update_with_defect(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param)
	...
	for(s = 0; s < ((param->d_N_replica_pt) * (param->d_volume)); s++)
		...
		for(dir = 0; dir < STDIM; dir++) { unitarize(&(GC[i].lattice[r][dir])); }
		}
	}
...
void parallel_tempering_with_hierarchical_update(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param, Rectangle const *const most_update, Rectangle const *const clover_rectangle, Rectangle const *const swap_rectangle, Acc_Utils *acc_counters)
	...
	if(param->d_N_replica_pt > 1)
		...
		conf_translation(&(GC[0]), geo, param);
		if(param->d_N_hierarc_levels > 0) hierarchical_update_rectangle_with_defect(GC, geo, param, start_hierarc, most_update, clover_rectangle, swap_rectangle, acc_counters);
		}

	// increase update index of all replica
	for(i = 0; i < param->d_N_replica_pt; i++) GC[i].update_index++;
	}
...
void update_with_trace_def(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param, double *acc)
	...
	#endif
	for(r = 0; r < param->d_space_vol; r++) { asum += (long)a[r]; }
...
void cooling(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param, int n)
	...
	for(r = 0; r < (param->d_volume); r++)
		{
		for(i = 0; i < STDIM; i++) { unitarize(&(GC->lattice[r][i])); }
		}
	}
	...
	if(max_dist < param->d_agf_delta) //if the integration step is accepted, advance t
		...
		*accepted = 0;
		for(dir = 0; dir < STDIM; dir++)
			...
			#pragma omp parallel for num_threads(NTHREADS) private(r)
			#endif
			for(r = 0; r < param->d_volume; r++) { equal(&(GC->lattice[r][dir]), &(GC_old->lattice[r][dir])); }
			}
		}
...
void ape_smearing(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param, double alpha, int n)
	...
	if(n > 0 && n % 2 == 0) // GC=helper1
		...
		for(dir = 0; dir < STDIM; dir++)
			...
			#pragma omp parallel for num_threads(NTHREADS) private(r)
			#endif
			for(r = 0; r < param->d_volume; r++) { equal(&(GC->lattice[r][dir]), &(helper1.lattice[r][dir])); }
			}
		}
