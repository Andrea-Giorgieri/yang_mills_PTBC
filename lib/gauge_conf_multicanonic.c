...
void init_multicanonic_gauge_conf(Gauge_Conf *const GC, Geometry const *const geo, GParam const *const param)
	...
	for(long s = 0; s < (param->d_volume); s++)
		{
	...
	GAUGE_GROUP **aux = GC->lattice_cold;
...
void init_multicanonic_acc_utils(Acc_Utils *acc_counters, GParam const *const param)
	...
	if((acc_counters->multicanonic_acc_filep) != NULL) // file exists
		...
		for(int i = 0; i < STDIM; i++) fprintf(acc_counters->multicanonic_acc_filep, "%d ", param->d_size[i]);
...
void print_multicanonic_acceptance(Gauge_Conf const *const GC, GParam const *const param, Acc_Utils const *const acc_counters)
	...
	if(GC->update_index % param->d_measevery == 0)
		...
		for(int i = 0; i < param->d_N_replica_pt; i++)
			{
			if(acc_counters->num_metro_multicanonic[i] != 0)
				mean_acc = (double) acc_counters->num_accepted_metro_multicanonic[i] / (double) acc_counters->num_metro_multicanonic[i];
			else
				mean_acc = 0.0;
...
static inline void update_topo_grid(double *grid, int const n_grid, double const grid_max, double const grid_step, double const x, double const s)
	{
	// index of grid interval containing x
	int const i_grid = (int) floor((x + grid_max) / grid_step);
	...
	else if(i_grid >= n_grid - 1)
		{
		grid[n_grid - 1] += s;
		}
	else
		{
		double const x0 = i_grid * grid_step - grid_max;
		double const t = (x - x0) / grid_step;
...
void tune_topo_potential(Gauge_Conf const *const GC, GParam *const param, Tune_Utils *const tune_utils)
	{
	int const n_replica = param->d_N_replica_pt;
	for(int a = 0; a < n_replica; a++)
		{
		// get stored_topcharge x
		double x = GC[a].stored_topcharge;
		if(param->d_topo_alpha > MIN_VALUE)
			x = round(param->d_topo_alpha * x);

		// get tuning step
		double const s = tune_utils->tuning_stp[a];

		// update V_a(x)
		update_topo_grid(param->d_grid[a], param->d_n_grid, param->d_grid_max, param->d_grid_step, x, s);

		// update V_a(-x) to enforce even potential (V_a(0) updated twice as intended)
		if(param->d_topo_tuning_even)
			update_topo_grid(param->d_grid[a], param->d_n_grid, param->d_grid_max, param->d_grid_step, -x, s);

		// update counters
		tune_utils->sweep_counter[a] += 1 - tune_utils->sweep_check[a];

		int const i_grid = (int) floor((x + param->d_grid_max) / param->d_grid_step);
		int const flag = tune_utils->sweep_flag[a];
	...
	// normalize V_a(0) = 0 for all replicas
	int const i_grid_0 = (int) floor(param->d_grid_max / param->d_grid_step);
	for(int a = 0; a < n_replica; a++)
		{
		double const offset = param->d_grid[a][i_grid_0];
		for(int i = 0; i < param->d_n_grid; i++)
			param->d_grid[a][i] -= offset;
		}
	}

int update_tuning_stp(Tune_Utils *const tune_utils,
                      GParam const *const param)
	{
	int const n_replica = param->d_N_replica_pt;

	// check that all replicas have swept the topological potential,
	// return 0 otherwise
	for(int a = 0; a < n_replica; a++)
		{
	...
	// update tuning steps and reset sweep information
	double max_mean_dV = 0.0;
	for(int a = 0; a < n_replica; a++)
		{
		double const mean_dV = tune_utils->tuning_stp[a] * tune_utils->sweep_counter[a] / param->d_n_grid;

		if(mean_dV > max_mean_dV)
			max_mean_dV = mean_dV;

		double const factor = fmax(0.5, 1.0 - mean_dV / 3.0);

		tune_utils->tuning_stp[a] *= factor;
		tune_utils->sweep_counter[a] = 0;
		tune_utils->sweep_check[a] = 0;
		}

	// return -1 if tuning is completed, else return 1
	return (max_mean_dV < param->d_topo_tuning_thr) ? -1 : 1;
	}
...
double compute_topo_potential(int const a, double x, GParam const *const param)
	{
	// alpha-rounding of topological charge
	if(param->d_topo_alpha > MIN_VALUE)
		x = round(param->d_topo_alpha * x);
	...
	// inside grid, linear interpolation: V(x) = V(x0) + [dV/dx|(x0)] (x-x0)
	if(i_grid >= 0 && i_grid < param->d_n_grid - 1)
		{
		double const x0 = i_grid * param->d_grid_step - param->d_grid_max;
		double const t = (x - x0) / param->d_grid_step;
		return param->d_grid[a][i_grid] * (1.0 - t) + param->d_grid[a][i_grid + 1] * t;
		}

	// outside grid, saturate to extreme values
	return (i_grid < 0) ? param->d_grid[a][0] : param->d_grid[a][param->d_n_grid - 1];
	}
...
void compute_topo_potential_debug(char *filename, double x_min, double x_max, double x_stp, GParam const *const param)
	...
	for(double x = x_min; x <= x_max; x += x_stp)
		{
		// write x
		fprintf(fp, "% 12.6e ", x);
		...
		for(int a = 0; a < param->d_N_replica_pt; a++)
			{
			double const V = compute_topo_potential(a, x, param);
			fprintf(fp, "% 12.6e ", V);
			}
		fprintf(fp, "\n");
		}
...
double multicanonic_delta_loc_topcharge(Gauge_Conf const *const GC1,
	...
	for(int i = 0; i < 3; i++)
		...
		double real1 = retr(&aux3) * NCOLOR;

		times(&aux3, &aux2, &aux1); // aux3 = aux2 * aux1
		double real2 = retr(&aux3) * NCOLOR;
...
double multicanonic_delta_topcharge_rectangle(Gauge_Conf const *const GC,
	...
	#endif
	for(long n = 0; n < (topcharge_rect->d_vol_rect); n++)
		{
		long r = topcharge_rect->rect_sites[n];
		res += multicanonic_delta_loc_topcharge(GC, &helper, geo, param, r);
		}
	...
	for(n=0; n<(param->d_volume); n++)
		{
		res += multicanonic_delta_loc_topcharge(GC, &helper, geo, param, n);
		int is_on_rect=0;
		long r;
...
double topcharge_agf_multicanonic(Gauge_Conf *const GC,
	...
	if(param->d_topo_agf_time > 0.0) //if using gradient flow
		...
		double const charge = topcharge(GC, geo, param);
		restore_gauge_conf(GC, param);

		return charge;
		}
	return topcharge(GC, geo, param); // no gradient flow
	}
...
double metropolis_prob_multicanonic(int const a, double const Q_new, double const Q_old, GParam const *const param)
	{
	double const V_old = compute_topo_potential(a, Q_old, param);
	double const V_new = compute_topo_potential(a, Q_new, param);
...
double delta_topo_potential_swap(Gauge_Conf const *const GC, int const a, int const b, GParam const *const param)
	{
	double const Q_a = GC[a].stored_topcharge;
	double const Q_b = GC[b].stored_topcharge;

	// before swap V_a(Q_a) + V_b(Q_b) , after swap V_a(Q_b) + V_b(Q_a)
	double const V_noswap = compute_topo_potential(a, Q_a, param) + compute_topo_potential(b, Q_b, param);
	double const V_swap = compute_topo_potential(a, Q_b, param) + compute_topo_potential(b, Q_a, param);
...
int multicanonic_metropolis_step_all_links(Gauge_Conf *const GC, Geometry const *const geo, GParam const *const param)
	...
	double const Q_old = GC->stored_topcharge;
	double Q_new;
	switch(param->d_topo_cooling)
		{
		case 0: // topcharge not cooled
	...
	// Metropolis test, p >= 1: always accept; p < 1: accept with probability p
	int acc = 1;
	double const p = metropolis_prob_multicanonic(GC->replica_index, Q_new, Q_old, param);
	if(p < 1)
		{
		if(casuale() > p) acc = 0;
		}
...
int multicanonic_metropolis_step_rectangle(Gauge_Conf *const GC, Geometry const *const geo, GParam const *const param,
	...
	switch(param->d_topo_cooling)
		...
		case 1: // topcharge cooled
			...
			break;
		default:
			REQUIRE(0, "unsupported cooling method %d", param->d_topo_cooling);
		}
	// TODO: debug, remove
