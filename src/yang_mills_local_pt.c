...
// to disable nested parallelism
	...
	for(count = 0; count < param.d_sample; count++)
		...
		if(GC[0].update_index % param.d_measevery == 0 && GC[0].update_index >= param.d_thermal) { perform_measures_localobs_cooling(&(GC[0]), &geo, &param, datafilep, chiprimefilep, topchar_tprof_filep); }
